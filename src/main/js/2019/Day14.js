function readInput(s) {
    const splitToken = token => {
        ...
        return [parseInt(tokens[0].trim()), tokens[1].trim()]
    }

    const raw = s || `1 RNQHX, 1 LFKRJ, 1 JNGM => 8 DSRGV
    ...
    return raw.split('\n')
        .map(row => row.split(' => '))
        .map(([left, right]) => {
            return [left.split(', ').map(splitToken), splitToken(right)];
        })
}

function calculate(reactions, n, chemical) {
    if (chemical === 'ORE') {
        return n
    }
    console.info('calculate', n, chemical)
    return reactions
        .filter(reaction => reaction[1][1] === chemical)
        .map(([inputs, [outputN]]) => {
            const factor = Math.ceil(n / outputN)
            return inputs
