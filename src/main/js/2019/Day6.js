...
while (paths.filter(path => path[0] !== 'COM').length > 0) {
    paths = paths.map((path, _, paths) => {
        const first = path[0]
        if (first === 'COM')
            return path

        const prefix = paths.find(other => other.indexOf(first) > 0)
        if (!prefix) {
            return path
        }

        return [...prefix.slice(0, prefix.indexOf(first)), ...path]
    })
}
