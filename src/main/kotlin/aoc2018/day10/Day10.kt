...
fun main() {
    ...
    var counter = 0
    while (true) {
        val moved = data.map { (p, v) -> Point(p.x + v.x, p.y + v.y) to v }
        if (moved.measure() >= data.measure()) {
...
private fun List<Pair<Point, Speed>>.measure() =
    with(map { it.first }) { maxOf { it.x } - minOf { it.x } + maxOf { it.y } - minOf { it.y } }

data class Point(val x: Int, val y: Int)
data class Speed(val x: Int, val y: Int)

fun List<Pair<Point, Speed>>.asString() = with(map { it.first }) {
    (minOf { it.y }..maxOf { it.y })
        .joinToString("\n") { y ->
            (minOf { it.x }..maxOf { it.x })
                .joinToString("") { x -> if (contains(Point(x, y))) "#" else "." }
        }
}

fun input() = """position=<-42346,  10806> velocity=< 4, -1>
...
position=< 21492,  42728> velocity=<-2, -4>"""
    .split("\n")
    .map { Regex("""position=<\s*(-?\d+),\s*(-?\d+)> velocity=<\s*(-?\d+),\s*(-?\d+)>""").find(it)!!.destructured }
    .map { groups -> groups.toList().map { it.toInt() } }
    .map { (x, y, vx, vy) -> Pair(Point(x, y), Speed(vx, vy)) }