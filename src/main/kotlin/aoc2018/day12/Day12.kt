...
fun Pots.simulateGrowth(rules: Rules, turns: Long): Long {
    ...
    for (turnsLeft in turns downTo 1)
        pots = pots.widerRange()
            .filter { it.growsWith(pots, rules) }
            .also { nextPots ->
                if (nextPots.shape() == pots.shape())
                    return pots.score() + turnsLeft * (nextPots.score() - pots.score())
            }
    return pots.score()
}
...
private fun Iterable<Long>.matches(pots: Pots, p: Long) =
    (-2L..2L).all { i -> (i in this) == ((p + i) in pots) }
