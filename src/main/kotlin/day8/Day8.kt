...
fun main() {
    listOf(
        ...
        """"aaa\"aaa"""",
        """"\x27""""
    ).forEach { println("real length of $it: ${it.realLength()} vs. ${it.length}") }

    input()
        .split("\n")
        .sumOf { it.length - it.realLength() }
    ...
    listOf(
        ...
        """"aaa\"aaa"""",
        """"\x27""""
    ).forEach { println("encoded length of $it: ${it.encodedLength()} vs. ${it.length}") }

    input()
        .split("\n")
        .sumOf { it.encodedLength() - it.length }
        .also { println("result encoded $it") }
}

fun String.realLength() = substring(1, length - 1)
    .replace("""\"""", "/")
    .replace("""\\""", "/")
    .let { it.length - (it.split("""\\x""".toRegex()).size - 1) * 3 }

fun String.encodedLength() = replace("""\""", """\\""")
    .replace("\"", "\\\"")
    .let { it.length + 2 }

fun input() = """"azlgxdbljwygyttzkfwuxv"
