...
fun main() {
    ...
    input()
        .filterValues { sue ->
            lookingFor.all { (requiredKey, requiredValue) ->
                val value = sue[requiredKey] ?: return@all true
                value == requiredValue
            }
        }
        .also { println("fake aunt sue: ${it.keys.first()}") }
    ...
    input()
        .filterValues { sue ->
            lookingFor.all { (requiredKey, requiredValue) ->
                val value = sue[requiredKey] ?: return@all true
                when (requiredKey) {
                    "cats", "trees" -> value > requiredValue
                    "pomeranians", "goldfish" -> value < requiredValue
                    else -> value == requiredValue
                }
            }
        }
        .also { println("real aunt sue: ${it.keys.first()}") }
}

private fun lookingFor() = """children: 3
cats: 7
samoyeds: 2
pomeranians: 3
akitas: 0
...
perfumes: 1"""
    .split("\n")
    .associateIntProperties()


private fun input() = """Sue 1: goldfish: 6, trees: 9, akitas: 0
...
Sue 500: perfumes: 4, cars: 9, trees: 4"""
    .split("\n")
    .associate { row ->
        val (namePart, dataPart) = row.splitInTwoAround(": ")
        val sueNumber = namePart.substring("Sue ".length).toInt()
        val properties = dataPart
            .split(", ".toRegex())
            .associateIntProperties()
        Pair(sueNumber, properties)
    }

fun String.splitInTwoAround(x: String): Pair<String, String> {
    return indexOf(x).let { Pair(substring(0, it), substring(it + x.length)) }
}

fun List<String>.associateIntProperties() =
    associate { tokens ->
        tokens.split(": ".toRegex()).let { Pair(it[0], it[1].toInt()) }
    }