...
fun main() {
    ...
    println(squires.map { (squire, plan) -> squire to plan.score(10) }.sortedByDescending { it.second }.joinToString("") { it.first })
    ...
    val knightsResult = squires.map { (squire, plan) ->
        var power = 10
        var sum = 0
        repeat(knightsTrack.length * 10) { i ->
            val track = knightsTrack[i % knightsTrack.length].takeIf { it == '-' || it == '+' }
            when (track ?: plan[i % plan.size]) {
                "+" -> power += 1
                "-" -> power -= 1
            }
            sum += power
    ...
    println(knightsResult.sortedByDescending { it.second }.joinToString("") { it.first })
    ...
    while (true) {
        ...
        finalTrack += finalTrackMap[curr.y][curr.x].toString()
    }
