    ...
    fun String.solve(numbers: List<Int>): Long {
        ...
        return modified.sumOf {
            ...
            if (completedNumbers != numbers.take(completedNumbers.size)) 0
                ...
                val nextString = it.drop(completedString.length)
                val nextNumbers = numbers.drop(completedNumbers.size)
                cache[nextString to nextNumbers]
                    ?: nextString.solve(nextNumbers).also { cache[nextString to nextNumbers] = it }
            }
        }
    }
