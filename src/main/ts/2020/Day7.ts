...
pale magenta bags contain 1 clear gold bag, 5 posh fuchsia bags, 2 faded cyan bags.`
    ...
    .map(([container, content]) => ({
        color: removeBags(container),
        content: content === 'no other bags.' ? [] : content.split(", ").map(removeBags)
    }))
    .reduce((agg, x) => {
        agg.set(x.color, x.content.map(s => {
            const n = parseInt(s.substr(0, s.indexOf(' ')))
...
function containsShinyGold(color: string, rules: Map<string, Array<{n: number, color: string}>>): boolean {
    ...
    for (const content of rules.get(color)) {
        if (content.color === 'shiny gold' || containsShinyGold(content.color, rules)) {
            return true
        }
    }
    return false
...
let count = 0
for (const color of input.keys()) {
    if (containsShinyGold(color, input)) {
        count++
    }
}
...
function addBags(color: string, n: number): number {
    let count = n
    for (const x of input.get(color)) {
        count += n * addBags(x.color, x.n)
    }
    return count
}

console.log(addBags('shiny gold', 1) - 1)