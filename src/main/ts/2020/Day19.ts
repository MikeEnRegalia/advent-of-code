...
function test(input: string) {
    ...
    const rules = parts[0].split('\n')
        .map(a => a.split(':'))
        .reduce((acc, tokens) => acc.set(tokens[0].trim(), tokens[1].trim()), new Map<string, string>())
    ...
    const regEx = new RegExp('^' + resolve(rules, '0') + '$')

    return {
        regEx,
        messages: messages.map(m => regEx.exec(m)).filter(x => x).length
    }
}

console.info(test(`0: 4 1 5
1: 2 3 | 3 2
2: 4 4 | 5 5
3: 4 5 | 5 4
4: "a"
