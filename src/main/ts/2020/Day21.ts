...
function allergenFree(input: string) {
    ...
    while (map.size < allergens.length) {
        allergens
            .filter(a => !map.has(a))
            .forEach(allergen => {
    ...
    const ingredients = mapping.reduce((agg, {ingredients}) => {
        ingredients.filter(a => agg.indexOf(a) === -1).forEach(a => agg.push(a))
        return agg
    }, [])

    const safeIngredients = ingredients.filter(i => Array.from(map.values()).indexOf(i) === -1)

    const n = mapping.map(m => m.ingredients).reduce((agg, x) => {
        return agg + x.filter(i => safeIngredients.indexOf(i) > -1).length
    }, 0)


    return {
        n,
        cdil: Array.from(map.entries()).sort((a, b) => a[0] < b[0] ? -1 : 1).map(x => x[1]).join(',')
    }
}

console.info(allergenFree(`mxmxvkd kfcds sqjhc nhms (contains dairy, fish)
trh fvjkl sbzzf mxmxvkd (contains dairy)
sqjhc fvjkl (contains soy)
sqjhc mxmxvkd sbzzf (contains fish)`))

console.info(allergenFree(input()))

function input() {
    return `rtdhb jxspzl bjgjgb tjtgbf jdmsljx psjq jqktb qjtlsg trlx vpzxk rfccb lccbpx nrz qndq vnml cvzqq bdt sqqr tpfj rczt xdpvd mpms npk tgpv tvcrkml tclx bkgmcsx fbvsh rjdqt hdcj rjjp lggrq vxnd xhdf tgkgzqj dgxrzglk grqfjk kgckc rthjg hbnf clnkzp zqtd vndnsd jspkl rbxklnd cdvjskn vblsdx lxxvb chkqhd vdnnrpc vzqmj (contains wheat)
