export default {}
const puzzleInput = `eyr:2028 iyr:2016 byr:1995 ecl:oth
...
console.log(puzzleInput.filter(hasRequired).length)

function hasRequired(data) {
    return required.every(field => data.has(field))
}

const validators = {
    byr: v => within(v, 1920, 2002),
    iyr: v => within(v, 2010, 2020),
...
const check = (data: Map<string, string>) => {
...
console.log(puzzleInput.filter(hasRequired).filter(check).length)