([
    ...
    '5 + (8 * 3 + 9 + 3 * 4 * 3)',
    '5 * 9 * (7 * 3 * 3 + 9 * 3 + (8 + 6 * 4))',
]).forEach(s => parse(s).forEach(r => console.info(r.input + ': ', r.result)))

//console.info(parse(input()).reduce((acc, r) => {
//    return acc + r.result
//}, 0))
...
function parse(input: string) {
    return input
        .split('\n')
        .map(s => s.replace(/ /g, ''))
        .map(s => ({input: s, result: term(s)}))
}

function term(input: string): number {
    console.info(input)

    if (input.startsWith('(')) {
        return term(input.substr(1))
    }
    ...
    if (close > -1 && (open === -1 || close < open)) {
        return term(`${term(input.substr(0, close)).valueOf()}${input.substr(close + 1)}`)
    }

    const plus = input.indexOf('+')
    const times = input.indexOf('*')
    if (plus > -1 || times > -1) {
        if (plus > -1 && (times === -1 || plus < times)) {
            return term(input.substr(0, plus)) + term(input.substr(plus + 1))
        }
        return term(input.substr(0, times)) * term(input.substr(times + 1))
    }
